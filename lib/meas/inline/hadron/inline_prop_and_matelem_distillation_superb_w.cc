...
void read(XMLReader &xml, const std::string &path,
  ...
  input.frozen_boundary_sidecar_file.clear();
  if (inputtop.count("frozen_boundary_sidecar_file") == 1) {
    read(inputtop, "frozen_boundary_sidecar_file",
         input.frozen_boundary_sidecar_file);
  }

  input.frozen_boundary_child_id = -1;
  if (inputtop.count("frozen_boundary_child_id") == 1) {
    read(inputtop, "frozen_boundary_child_id", input.frozen_boundary_child_id);
  }

  input.factorized_intermediate_file.clear();
  if (inputtop.count("factorized_intermediate_file") == 1) {
    read(inputtop, "factorized_intermediate_file",
         input.factorized_intermediate_file);
  }

  input.factorized_cross_file.clear();
...
void write(XMLWriter &xml, const std::string &path,
  ...
  if (!input.frozen_boundary_sidecar_file.empty()) {
    write(xml, "frozen_boundary_sidecar_file",
          input.frozen_boundary_sidecar_file);
  }
  if (input.frozen_boundary_child_id >= 0) {
    write(xml, "frozen_boundary_child_id", input.frozen_boundary_child_id);
  }
  if (!input.factorized_intermediate_file.empty()) {
    write(xml, "factorized_intermediate_file",
          input.factorized_intermediate_file);
  }
...
//! Propagator input
void read(XMLReader &xml, const std::string &path,
          Params::Param_t::Phasing_t &input) {
  XMLReader inputtop(xml, path);

  read(inputtop, "sink", input.snk);
  if (input.snk.size() != Nd - 1) {
    throw std::runtime_error(std::string("phase sink should have ") +
                             std::to_string(Nd - 1) + " components");
  }
  read(inputtop, "source", input.src);
  if (input.src.size() != Nd - 1) {
    throw std::runtime_error(std::string("phase source should have ") +
                             std::to_string(Nd - 1) + " components");
  }
}
...
//! Propagator output
void write(XMLWriter &xml, const std::string &path,
           const Params::Param_t::Phasing_t &input) {
  push(xml, path);
...
void read(XMLReader &xml, const std::string &path,
  ...
  if (inputtop.count("phases") == 1) {
    ...
    if (input.phase.size() != Nd - 1) {
      throw std::runtime_error(std::string("phase tag should have ") +
                               std::to_string(Nd - 1) + " components");
    }
  } else {
  ...
  if (inputtop.count("use_device_for_contractions") == 1) {
    read(inputtop, "use_device_for_contractions",
         input.use_device_for_contractions);
  }
...
struct FactorizedPropGeometry {
  ...
  multi1d<GaugeSubdomainSplitInterval> active_local_intervals;
  multi1d<int> gamma_star_local_slices;
  multi1d<int> gamma_local_slices;
  GaugeSubdomainSplitPlan plan;

  FactorizedPropGeometry() : enabled(false), child_id(-1), t_dir(-1) {}
};

bool factorizedPropRequested(const Params::NamedObject_t &named_obj) {
  return !named_obj.frozen_boundary_sidecar_file.empty() ||
         named_obj.frozen_boundary_child_id >= 0 ||
         !named_obj.factorized_intermediate_file.empty() ||
         !named_obj.factorized_cross_file.empty();
}

void appendIntervalSlices(const multi1d<GaugeSubdomainSplitInterval> &intervals,
                          multi1d<int> &slices) {
  int total = 0;
  for (int i = 0; i < intervals.size(); ++i)
    total += intervals[i].t_end - intervals[i].t_start + 1;

  slices.resize(total);
  int p = 0;
  for (int i = 0; i < intervals.size(); ++i) {
    for (int t = intervals[i].t_start; t <= intervals[i].t_end; ++t)
      slices[p++] = t;
  }
}

multi1d<GaugeSubdomainSplitInterval> buildActiveIntervals(
    const multi1d<GaugeSubdomainSplitInterval> &frozen_intervals,
    int child_extent) {
  std::vector<GaugeSubdomainSplitInterval> active;
  int next_start = 0;

  for (int i = 0; i < frozen_intervals.size(); ++i) {
    if (next_start < frozen_intervals[i].t_start) {
      GaugeSubdomainSplitInterval interval;
      interval.t_start = next_start;
      interval.t_end = frozen_intervals[i].t_start - 1;
      active.push_back(interval);
    }

    next_start = frozen_intervals[i].t_end + 1;
  }

  if (next_start < child_extent) {
    GaugeSubdomainSplitInterval interval;
    interval.t_start = next_start;
    interval.t_end = child_extent - 1;
    active.push_back(interval);
  }

  multi1d<GaugeSubdomainSplitInterval> result(active.size());
...
FactorizedPropGeometry
loadFactorizedPropGeometry(const Params::NamedObject_t &named_obj,
                           int decay_dir) {
  FactorizedPropGeometry geometry;
  if (!factorizedPropRequested(named_obj))
    return geometry;

  if (named_obj.frozen_boundary_sidecar_file.empty()) {
    throw std::runtime_error("factorized propagator output requested without "
                             "frozen_boundary_sidecar_file");
  }

  if (named_obj.frozen_boundary_child_id < 0 ||
      named_obj.frozen_boundary_child_id > 1) {
    throw std::runtime_error(
        "frozen_boundary_child_id must be 0 or 1 when factorized "
        "propagator output is requested");
  }

  GaugeSubdomainSplitPlan plan;
  XMLReader sidecar_in(named_obj.frozen_boundary_sidecar_file);
  read(sidecar_in, "/GaugeSubdomainSplitInfo", plan);

  if (plan.param.t_dir != decay_dir) {
    std::ostringstream os;
    os << "factorized propagator decay_dir=" << decay_dir
       << " does not match sidecar t_dir=" << plan.param.t_dir;
    throw std::runtime_error(os.str());
  }

  geometry.enabled = true;
  geometry.sidecar_file = named_obj.frozen_boundary_sidecar_file;
  geometry.child_id = named_obj.frozen_boundary_child_id;
  geometry.t_dir = plan.param.t_dir;
  geometry.plan = plan;

  if (geometry.child_id == 0) {
    geometry.child_nrow = plan.child0_nrow;
    geometry.local_to_global_t = plan.child0_local_to_global_t;
    geometry.frozen_local_intervals = plan.child0_frozen_local_intervals;
  } else {
    geometry.child_nrow = plan.child1_nrow;
    geometry.local_to_global_t = plan.child1_local_to_global_t;
    geometry.frozen_local_intervals = plan.child1_frozen_local_intervals;
  }

  geometry.active_local_intervals = buildActiveIntervals(
      geometry.frozen_local_intervals, geometry.child_nrow[geometry.t_dir]);
  appendIntervalSlices(geometry.frozen_local_intervals,
                       geometry.gamma_star_local_slices);
  appendIntervalSlices(geometry.active_local_intervals,
                       geometry.gamma_local_slices);

  return geometry;
}

void writeFactorizedPropGeometry(XMLWriter &xml,
                                 const FactorizedPropGeometry &geometry) {
  push(xml, "FactorizedPropGeometry");
  write(xml, "enabled", geometry.enabled);
  if (geometry.enabled) {
    write(xml, "sidecar_file", geometry.sidecar_file);
    write(xml, "child_id", geometry.child_id);
    write(xml, "t_dir", geometry.t_dir);
    write(xml, "child_nrow", geometry.child_nrow);
    write(xml, "local_to_global_t", geometry.local_to_global_t);
    write(xml, "frozen_local_intervals", geometry.frozen_local_intervals);
    write(xml, "active_local_intervals", geometry.active_local_intervals);
    write(xml, "gamma_star_local_slices", geometry.gamma_star_local_slices);
    write(xml, "gamma_local_slices", geometry.gamma_local_slices);
  }
  pop(xml);
}
...
// Returns a copy of u with all time-direction links on t_slice set to zero.
// t_dir defaults to Nd-1, Chroma's usual convention for the time direction.
// helper for open boundary conditions
multi1d<LatticeColorMatrix>
zeroTemporalLinksOnSlice(const multi1d<LatticeColorMatrix> &u, int t_slice,
                         int t_dir = Nd - 1) {
  multi1d<LatticeColorMatrix> u_out(u.size());
  for (int mu = 0; mu < u.size(); ++mu)
    u_out[mu] = u[mu];

  // Boolean mask: true on sites whose coordinate in t_dir equals t_slice
  LatticeInteger t_coord = Layout::latticeCoordinate(t_dir);
  LatticeBoolean on_slice = (t_coord == t_slice);

  LatticeColorMatrix zero_mat = zero;

  u_out[t_dir] = where(on_slice, zero_mat, u[t_dir]);
...
LatticeFermion restrictToTimeslices(const LatticeFermion &psi,
  ...
  LatticeBoolean on_boundary = false; // starts false everywhere
  for (int i = 0; i < keep_tslices.size(); ++i)
...
/// \param chi: lattice color tensor, cxyztXn
/// \param t_source: first time-slice in chi

inline std::vector<std::shared_ptr<LatticeFermion>>
toLatticeFermions(const Tensor<Nd + 3, SB::Complex> &chi, int t_source) {
  const auto n = chi.kvdim().at('n');
  std::vector<std::shared_ptr<LatticeFermion>> r(n);
  for (int col = 0; col < n; col++) {
    chis[col].reset(new LatticeFermion);
    *chis[col] = zero;
    chi.kvslice_from_size({{'n', col}}, {{'n', 1}})
        .copyTo(
            SB::asTensorView(*chis[col]).kvslice_from_size({{'t', t_source}}));
  }
  return r;
}

/// Return an SB::Tensor from the given vector of LatticeFermion
/// \param chi: vector of LatticeFermion
/// \param t_source: first time-slice to return
/// \param num_tslices: number of timeslices to return

inline Tensor<Nd + 3, SB::Complex>
toSBTensor(const std::vector<std::shared_ptr<LatticeFermion>> &chi,
  ...
  for (int col = 0; col < chi.size(); col++) {
    SB::asTensorView(*chis[col])
        .kvslice_from_size({{'t', t_source}}, {{'t', num_tslices}})
        .copyTo(r.kvslice_from_size({{'n', col}}, {{'n', 1}}));
  }
  return r;
}
...
void InlineMeas::func(unsigned long update_no, XMLWriter &xml_out) {
  ...
  std::map<int, std::set<int>>
      phasing_pairs; ///< phasing src -> list of phasing sink
  if (params.param.contract.phase.size() > 0) {
    ...
    if (!params.param.contract.use_superb_format) {
      throw std::runtime_error(
          "only the new format has support for `phases'; please set "
          "'use_superb_format' to true");
    }
  ...
  // TODO: Stitch child configs together for full geometry
  ...
  const FactorizedPropGeometry factorized_geometry =
      loadFactorizedPropGeometry(params.named_obj, decay_dir);
  writeFactorizedPropGeometry(xml_out, factorized_geometry);

  //
  // Read in the source along with relevant information.
  //

  SB::ColorvecsStorage colorvecsSto =
      SB::openColorvecStorage(params.named_obj.colorvec_files);
  ...
  if (!params.param.contract.use_superb_format) {
    ...
    // NOTE: metadata_xml only has a valid value on Master node; so do a
    // broadcast
    ...
    st = SB::StorageTensor<8, SB::ComplexD>(
        ...
        params.param.contract.output_file_is_local ? SB::LocalFSFile
                                                   : SB::SharedFSFile);
  }
  ...
  try {
    ...
    // Enforce open boundary conditions through gauge link zeroing
    if (factorized_geometry.enabled) {

      FactorizedPropGeometry geometry =
          loadFactorizedPropGeometry(named_obj, decay_dir);
      // Zero timeslices where the active zone reaches the boundary
      multi1d<LatticeColorMatrix> u_sep = u;
      for (int i = 0; i < geometry.child0_frozen_local_intervals.size(); i++) {
        // TODO:Handling for end of lattice below?
        multi1d<LatticeColorMatrix> u_sep = zeroTemporalLinksOnSlice(
    ...
    SB::DeviceHost dev = params.param.contract.use_device_for_contractions
                             ? SB::OnDefaultDevice
                             : SB::OnHost;
    ...
    if (factorized_geometry.enabled) {
      ...
      for (int tt = 0; tt < t_sources.size(); ++tt) {
        ...
        multi1d<int> inner_source_boundaries(4) inner_source_boundaries[0] = 0;
        ...
        // Create full fermionic vector for t_source
        ...
        for (const auto &it : phasing_pairs) {
          ...
          auto source_colorvec = SB::phaseColorvecs(
              colorvec.kvslice_from_size({{'t', t_source}}, {{'t', 1}}),
              t_source, phasings[phasing_src_idx]);
          for (int spin_source = 0; spin_source < Ns; ++spin_source) {
            // Contract source timeslice with D_{11}^-1
            // return all timeslices for now
            auto contract1_sb =
                SB::doInversion(PP, source_colorvec, t_source, 0, Lt,
            ...
            auto y_boundary1 = restrictToTimeslices(
            ...
            SB::doInversion(PP, contract2, y_boundary1, max_rhs);
            ...
            for (int phasing_snk_idx : it.second) {
              // Phase the sink
              auto colorvec_snk = SB::phaseColorvecs(colorvec, first_tslice,
                                                     phasings[phasing_snk_idx]);
              ...
              if (!params.param.contract.use_superb_format) {
                ...
                for (int i_tslice = 0; i_tslice < num_tslices; ++i_tslice) {
                  for (int spin_sink = 0; spin_sink < Ns; ++spin_sink) {
                    ...
                    key.t_slice =
                        SB::normalize_coor(i_tslice + first_tslice, Lt);
                    ...
                    if (local_elems) {
                      for (int colorvec_sink = 0; colorvec_sink < num_vecs;
                           ++colorvec_sink) {
                        for (int colorvec_source = 0;
                             colorvec_source < num_vecs; ++colorvec_source) {
                          std::complex<REAL> e =
                              local_elems.get({colorvec_sink, colorvec_source,
                                               spin_sink, 0, i_tslice});
                          val.mat(colorvec_sink, colorvec_source)
                              .elem()
                              .elem()
                              .elem() = RComplex<REAL64>(e.real(), e.imag());
                        }
                      }
                    }
            ...
            snarss1.stop();
            QDPIO::cout << "Time to store the props : "
                        << snarss1.getTimeInSeconds() << " secs" << std::endl;
          } // for spin
        } // for phasing
      } // for timesource
      ...
      for (int tt = 0; tt < t_sources.size(); ++tt) {
        ...
        // Compute the first tslice and the number of tslices involved in the
        // contraction
        int first_tslice = SB::normalize_coor(
            t_source - params.param.contract.Nt_backward, Lt);
        int num_tslices =
            std::min(params.param.contract.Nt_backward +
                         std::max(1, params.param.contract.Nt_forward),
                     Lt);

        // Get `num_vecs` colorvecs, and `num_tslices` tslices starting from
        // time-slice `first_tslice`
        SB::Tensor<Nd + 3, SB::Complex> colorvec =
            SB::getColorvecs<SB::Complex>(colorvecsSto, u, decay_dir,
                                          first_tslice, num_tslices, num_vecs,
                                          "cxyzXnt", SB::Coor<3>{{}}, dev);
        ...
        for (const auto &it : phasing_pairs) {
          ...
          // Get all eigenvectors for `t_source`
          auto source_colorvec = SB::phaseColorvecs(
              colorvec.kvslice_from_size(
                  {{'t', SB::normalize_coor(t_source - first_tslice, Lt)}},
                  {{'t', 1}}),
              t_source, phasings[phasing_src_idx]);

          for (int spin_source = 0; spin_source < Ns; ++spin_source) {
            // Invert the source for `spin_source` spin and retrieve
            // `num_tslices` tslices starting from tslice `first_tslice` NOTE: s
            // is spin source, and S is spin sink
            SB::Tensor<Nd + 5, SB::Complex> quark_solns = SB::doInversion(
                PP, source_colorvec, t_source, first_tslice, num_tslices,
                {spin_source}, max_rhs, "cxyzXnSst");
