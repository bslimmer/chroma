...
//Adding for Dirac operator handling in factorized measurement
#include "handle.h"
...
namespace Chroma {
	...
	namespace InlinePropAndMatElemDistillationSuperbEnv {
		...
		inline std::vector<std::shared_ptr<LatticeFermion>>
			toLatticeFermions(const SB::Tensor<Nd + 5, SB::Complex> &chi, int t_source) {
				const auto n = chi.kvdim().at('n');
				std::vector<std::shared_ptr<LatticeFermion>> r(n);
				for (int col = 0; col < n; col++) {
					r[col].reset(new LatticeFermion);
					*r[col] = zero;
					chi.kvslice_from_size({{'n', col}}, {{'n', 1}})
						.copyTo(
								SB::asTensorView(*r[col]).kvslice_from_size({{'t', t_source}}));
				}
				return r;
			}
