...
- This repo already includes the small Chroma-side compatibility change in [lib/actions/ferm/linop/eoprec_clover_orbifold_linop_w.cc](/Users/benslimmer/Documents/Physics%20Research/Frontier%20Work/DD/chroma-dd/chroma/lib/actions/ferm/linop/eoprec_clover_orbifold_linop_w.cc:171), so the remaining setup is in the build recipe.
...
7. build `t_temporal_zone_gaugebc` and `t_leapfrog`
8. run both tests
...
- `build-tests` also builds `t_temporal_zone_gaugebc` and `t_leapfrog`.
- `run-tests` runs both test executables after building them.
...
```bash
git submodule update --init other_libs/qdp-lapack
...
Build and run the two tests:

```bash
cmake --build /private/tmp/chroma-build/chroma-localbinarydb \
  --target t_temporal_zone_gaugebc t_leapfrog -j8
